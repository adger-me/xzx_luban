...
class Luban {
  ...
  static const MethodChannel _channel = MethodChannel('xzx_luban');
  ...
  static String? _lubanCompress(CompressObject object) {
    ...
    if (isJpg) {
      decodedImageFile = File(object.path! + '/img_${DateTime.now().millisecondsSinceEpoch}.jpg');
    } else if (isPng) {
      decodedImageFile = File(object.path! + '/img_${DateTime.now().millisecondsSinceEpoch}.png');
    } else {
      throw Exception("xzx_luban don't support this image type");
    }
